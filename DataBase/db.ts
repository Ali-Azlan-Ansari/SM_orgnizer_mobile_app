...
export const createMarksTable = async (db: SQLiteDatabase): Promise<void> => {
  const query = `
...
export const createScheduleTable = async (db: SQLiteDatabase): Promise<void> => {
  const query = `
...
// 3️⃣ GET ALL MARKS
export const getAllMarks = async (db: SQLiteDatabase): Promise<Mark[]> => {
  const query = `SELECT * FROM marks ORDER BY id DESC;`;
  const results = await db.executeSql(query);
  const marks: Mark[] = [];
  for (let i = 0; i < results[0].rows.length; i++) {
    marks.push(results[0].rows.item(i));
  }
  return marks;
};

// 4️⃣ GET MARK BY ID
export const getMarkById = async (db: SQLiteDatabase, id: number): Promise<Mark | null> => {
  const results = await db.executeSql(`SELECT * FROM marks WHERE id = ?;`, [id]);
  if (results[0].rows.length > 0) {
    return results[0].rows.item(0) as Mark;
  }
  return null;
};
...
// 6️⃣ DELETE MARK BY ID
export const deleteMarkById = async (db: SQLiteDatabase, id: number): Promise<void> => {
  await db.executeSql(`DELETE FROM marks WHERE id = ?;`, [id]);
};

// 7️⃣ DROP TABLE (for reset)
export const dropMarksTable = async (db: SQLiteDatabase): Promise<void> => {
  await db.executeSql(`DROP TABLE IF EXISTS marks;`);
};


export const insertSchedule = async (db: SQLiteDatabase, schedule: Schedule) => {
  const insertQuery = `INSERT INTO Schedule (day, subject, start_time, end_time) VALUES (?, ?, ?, ?)`;
...
// 🔹 get all
export const getSchedules = async (db: SQLiteDatabase): Promise<Schedule[]> => {
  const results = await db.executeSql(`SELECT * FROM Schedule`);
...
// 🔹 get by id
export const getScheduleById = async (db: SQLiteDatabase, id: number): Promise<Schedule | null> => {
  const results = await db.executeSql(`SELECT * FROM Schedule WHERE id = ?`, [id]);
  if (results[0].rows.length > 0) {
...
// 🔹 get by day
export const getSchedulesByDay = async (db: SQLiteDatabase, day: string): Promise<Schedule[]> => {
  const results = await db.executeSql(`SELECT * FROM Schedule WHERE day = ?`, [day]);
  const schedules: Schedule[] = [];
  results.forEach(result => {
    for (let i = 0; i < result.rows.length; i++) {
      schedules.push(result.rows.item(i));
...
// 🔹 update
export const updateSchedule = async (db: SQLiteDatabase, schedule: Schedule) => {
  if (!schedule.id) throw new Error('Schedule id required for update');
  const updateQuery = `UPDATE Schedule SET day = ?, subject = ?, start_time = ?, end_time = ? WHERE id = ?`;
...
// 🔹 delete
export const deleteSchedule = async (db: SQLiteDatabase, id: number) => {
