...
const EditClassTime = ({ route }: any): React.ReactElement => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const sch = await getScheduleById(db, id);
      if (sch) {
        setSubject(sch.subject);
        const idx = days.findIndex(d => d === sch.day);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      handlePress('Schedule updated', 2000, 'success');
